 ...
 * 不要开过多线程池，还是要共用
 * 
 * 线程池中处理任务的优先级为： 核心线程corePoolSize、任务队列workQueue、最大线程maximumPoolSize，如果三者都满了，
 * 使用handler处理被拒绝的任务。
 * 
 */
public class ThreadPoolUtil {

	public static int POOL_MULTIPLE = 20; // 默认线程
	...
	private static int MAX_POOL_MULTIPLE = POOL_MULTIPLE * (AVAILABLE_PROCESSPRS <= 0 ? 1 : AVAILABLE_PROCESSPRS); // 默认一个cpu
																													// n个线程

	public static int KEEP_ALIVE = 3; // when the number of threads is greater
										// than the core,
	...
	public static final ThreadPoolExecutor newPool(int P, int M, int K, BlockingQueue<Runnable> Q, RejectedExecutionHandler R) {
		...
		// 参数说明：最少数量 最大数量 空闲时间
		ThreadPoolExecutor pool = new ThreadPoolExecutor(P, // 处理线程数
				M, // 默认max线程
				K, // 存活线程
